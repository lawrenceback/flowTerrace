package com.jxgis.ffireserver.core.common.exception;

import com.jxgis.ffireserver.core.util.JSONHelper;
...
public class GlobalExceptionResolver implements HandlerExceptionResolver {
	 ...
	 * @return
	 */
	private ModelAndView processException(HttpServletRequest request, HttpServletResponse response, Object handler, Throwable deepestException, boolean isajax) {
		//步骤一、异常信息记录到日志文件中.
		log.error("全局处理异常捕获:", deepestException);
		//步骤二、异常信息记录截取前50字符写入数据库中.
		logDb(deepestException);
		//步骤三、分普通请求和ajax请求分别处理.
		if(isajax){
			return processAjax(request,response,handler,deepestException);
		}else{
			return processNotAjax(request,response,handler,deepestException);
		}
	}
	/**
	 * 异常信息记录截取前50字符写入数据库中
	...
	private ModelAndView processAjax(HttpServletRequest request,
		...
		ReturnBody json = new ReturnBody();
	...
	private ModelAndView processNotAjax(HttpServletRequest request,
	                                    HttpServletResponse response, Object handler, Throwable deepestException) {
		String exceptionMessage = getThrowableMessage(deepestException);
		Map<String, Object> model = new HashMap<String, Object>();
		model.put("exceptionMessage", exceptionMessage);
		model.put("ex", deepestException);
