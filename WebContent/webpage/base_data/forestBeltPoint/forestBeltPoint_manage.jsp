...
<head>
	...
	<link rel="stylesheet" href="${pageContext.request.contextPath}/plug-in-ui/css/global.css?3333">
...
<div class="ibox ibox-datagrid">
	...
	<div class="ibox-title">
		...
		<div class="ibox-tools ibox-tools-one">
			<a onclick="add()" ><i class="fa fa-plus"></i> 添加防火林带</a>
			...
			<ul class="dropdown-menu bg-warning" aria-labelledby="dLabel" id="cols">
				...
				<li><div class="checkbox i-checks"><label><input type="checkbox" checked property="build_addr"><i></i>营造位置</label></div></li>
				<li><div class="checkbox i-checks"><label><input type="checkbox" checked property="build_year"><i></i>营造年度</label></div></li>
				<li><div class="checkbox i-checks"><label><input type="checkbox" checked property="start_addr"><i></i>起始位置</label></div></li>
				<li><div class="checkbox i-checks"><label><input type="checkbox" checked property="stop_addr"><i></i>终止地点</label></div></li>
				<li><div class="checkbox i-checks"><label><input type="checkbox" checked property="tree_type"><i></i>树种</label></div></li>
				<li><div class="checkbox i-checks"><label><input type="checkbox" property="type"><i></i>类型</label></div></li>
				<li><div class="checkbox i-checks"><label><input type="checkbox" property="belt_len"><i></i>长度</label></div></li>
				<li><div class="checkbox i-checks"><label><input type="checkbox" property="belt_width"><i></i>宽度</label></div></li>
				...
				<li><div class="checkbox i-checks"><label><input type="checkbox" property="row_spacing"><i></i>株行距</label></div></li>
			...
			<div class="datagrid-talbe form-datagrid">
				<table class="table table-bordered table-hover" id="hot_list">
					...
					<tr>
						...
						<th class="sort" data-property="build_addr">营造位置<i class="iconfont icon-paixu"></i></th>
						<th class="sort" data-property="build_year">营造年度<i class="iconfont icon-paixu"></i></th>
						<th class="sort" data-property="start_addr">起始位置<i class="iconfont icon-paixu"></i></th>
						<th class="sort" data-property="stop_addr">终止地点	<i class="iconfont icon-paixu"></i>	</th>
						<th class="sort" data-property="tree_type">树种<i class="iconfont icon-paixu"></i></th>
						<th class="sort" data-property="type">类型<i class="iconfont icon-paixu"></i></th>
						<th class="sort" data-property="belt_len">长度<i class="iconfont icon-paixu"></i></th>
						<th class="sort" data-property="belt_width">宽度<i class="iconfont icon-paixu"></i></th>
						...
						<th class="sort" data-property="row_spacing">株行距<i class="iconfont icon-paixu"></i></th>
...
<script>
				...
				for (var i = 0; i < data_length; i++) {
					if (data.hasOwnProperty(i)) {
						var sub = temp.temp_hot_list.format({
							...
							name: data[i].name,
							build_addr: data[i].build_addr || '',
							build_year: data[i].build_year || '',
							start_addr: data[i].start_addr || '',
							stop_addr: data[i].stop_addr || '',
							tree_type: data[i].tree_type || '',
							type: data[i].type || '',
							shape: data[i].shape || '',
							belt_len: data[i].belt_len || '',
							belt_width: data[i].belt_width || '',
							row_spacing: data[i].row_spacing || '',
	...
	function add() {
		layer.open({
			...
			area: ['100%', '100%'],
			content: cg_vte.path + 'webpage/base_data/forestBeltPoint/forestBeltPoint_add.jsp',
			fix: false,
			scrollbar: false,
		});
	}
	function edit(item) {
		...
		layer.open({
			...
			maxmin: false, //开启最大化最小化按钮
			area: ['100%', '100%'],
			content: cg_vte.path + 'webpage/base_data/forestBeltPoint/forestBeltPoint_edit.jsp',
			btn: ['关闭'],
			fix: false,
			scrollbar: false,
	...
	function deletes() {
		...
		for (var i in ids){
			if (ids.hasOwnProperty(i)){
				params['beltPointList["'+i+'"].id'] = ids[i];
			}
		}

		$.ajax({
			url: cg_vte.path + 'if/forestBeltPointController/deleteForestBeltPoints',
			type: 'post',
			dataType: 'text',
			data: params,
			success: function (data) {
        ...
        for(var i in rows){
            if (rows.hasOwnProperty(i)){
                ...
                AMap.event.addListener(marker, 'click', function(event) {
                    ...
                    info.push("形状 : {shape}");
                    info.push("<br>");
                    info.push("起始地点 : {start_addr}");
                    info.push("<br>");
                    info.push("终止地点 : {stop_addr}");
                    info.push("<br>");
                    info.push("名称： {name}");
                    info.push("<br>");
                    info.push("营造年度： {build_year}");
                    info.push("<br>");
                    info.push("树种： {tree_type}");
                    info.push("</div>");
    ...
    function mapDatail() {
        ...
        content = content.format({
            ...
            stop_addr: data.stop_addr || '',
            name: data.name || '',
            build_addr: data.build_addr || '',
            build_year: data.build_year || '',
            tree_type: data.tree_type || '',
            type: data.type || '',
            build_unit: data.build_unit || '',
            belt_len: data.belt_len || '',
