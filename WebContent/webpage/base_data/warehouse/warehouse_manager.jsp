...
<head>
	...
	<link rel="stylesheet" href="${pageContext.request.contextPath}/plug-in-ui/css/global.css?222">
				...
				<div class="ibox ibox-datagrid">
					...
					<div class="ibox-title">
						...
						<div class="ibox-tools ibox-tools-one">
							<a onclick="add()" ><i class="fa fa-plus"></i> 添加储备库</a>
							<a onclick="mapView(this)" class="fa fa-map-o"> 地图模式</a>
							<a onclick="deletes(this)"> 批量删除</a>
							...
							<ul class="dropdown-menu bg-warning" aria-labelledby="dLabel" id="cols">
								...
								<li><div class="checkbox i-checks"><label><input type="checkbox" property="waveband"><i></i>波段</label></div></li>
								<li><div class="checkbox i-checks"><label><input type="checkbox" property="antdiameter"><i></i>天线直径</label></div></li>
								<li><div class="checkbox i-checks"><label><input type="checkbox" property="ip"><i></i>IP地址</label></div></li>
								<li><div class="checkbox i-checks"><label><input type="checkbox"   property="shape"><i></i>Geometry</label></div></li>
							...
							<div class="datagrid-talbe form-datagrid">
								...
								<table class="table table-bordered table-hover" id="hot_list">
									...
									<tr>
										...
										<th class="sort" data-property="waveband">波段<i class="iconfont icon-paixu"></i></th>
										<th class="sort" data-property="antdiameter">天线直径<i class="iconfont icon-paixu"></i></th>
										<th class="sort" data-property="ip">IP地址<i class="iconfont icon-paixu"></i></th>

										<th class="sort" data-property="shape">Geometry<i class="iconfont icon-paixu"></i></th>
...
<script>
				...
				for (var i = 0; i < data_length; i++) {
					if (data.hasOwnProperty(i)) {
						var sub = temp.temp_hot_list.format({
							...
							waveband: data[i].waveband,
							antdiameter: data[i].antdiameter,
							ip	: data[i].ip	,
							shape	: data[i].shape	 || '',
	...
	function add() {
		layer.open({
			...
			area: ['100%', '100%'],
			content: cg_vte.path + 'webpage/base_data/warehouse/warehouse_add.jsp',
			fix: false,
			scrollbar: false,
		});
	}
	function edit(item) {
		...
		layer.open({
			...
			maxmin: false, //开启最大化最小化按钮
			area: ['100%', '100%'],
			content: cg_vte.path + 'webpage/base_data/warehouse/warehouse_edit.jsp',
			btn: ['关闭'],
			fix: false,
			scrollbar: false,
	...
	function deletes() {
		...
		for (var i in ids){
			if (ids.hasOwnProperty(i)){
				params['warehouses["'+i+'"].id'] = ids[i];
			}
		}

		$.ajax({
			url: cg_vte.path + 'if/warehouseController/deleteSWarehouses',
			type: 'post',
			dataType: 'text',
			data: params,
			success: function (data) {
        ...
        for(var i in rows){
            if (rows.hasOwnProperty(i)){
                ...
                AMap.event.addListener(marker, 'click', function(event) {
                    ...
                    info.push("<br>");
                    info.push("类型编号 : {typeid}");
                    info.push("<br>");
                    info.push("所属编号： {user_id}");
                    info.push("<br>");
                    info.push("物资编号： {mat_id}");
