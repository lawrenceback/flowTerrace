			...
			<div class="ibox ibox-datagrid">
				...
				<div class="ibox-title">
					...
					<div class="ibox-tools ibox-tools-one">
						<a onclick="add()" ><i class="fa fa-plus"></i> 添加地面站</a>
...
<script>
	...
	function add() {
		layer.open({
			...
			area: ['100%', '100%'],
			content: cg_vte.path + 'webpage/base_data/satellitegroundstation/satellitegroundstation_add.jsp',
			fix: false,
			scrollbar: false,
		});
	}
	function edit(item) {
		...
		layer.open({
			...
			maxmin: false, //开启最大化最小化按钮
			area: ['100%', '100%'],
			content: cg_vte.path + 'webpage/base_data/satellitegroundstation/satellitegroundstation_edit.jsp',
			btn: ['关闭'],
			fix: false,
			scrollbar: false,
	...
	function deletes() {
		...
		for (var i in ids){
			if (ids.hasOwnProperty(i)){
				params['satelliteGroundStations["'+i+'"].id'] = ids[i];
			}
		}

		$.ajax({
			url: cg_vte.path + 'if/satelliteGroundStationController/deleteSatelliteGroundStations',
			type: 'post',
			dataType: 'text',
			data: params,
			success: function (data) {
        ...
        for(var i in rows){
            if (rows.hasOwnProperty(i)){
                ...
                AMap.event.addListener(marker, 'click', function(event) {
                    ...
                    info.push("<br>");
                    info.push("波段： {waveband}");
                    info.push("<br>");
                    info.push("天线直径： {antdiameter}");
                    info.push("</div>");
