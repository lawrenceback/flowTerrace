			...
			<div class="ibox ibox-datagrid">
				...
				<div class="ibox-title">
					...
					<div class="ibox-tools ibox-tools-one">
						<a onclick="add()" ><i class="fa fa-plus"></i> 林区危险及重要设施</a>
						...
						<ul class="dropdown-menu bg-warning" aria-labelledby="dLabel" id="cols">
							...
							<li><div class="checkbox i-checks"><label><input type="checkbox" property="content"><i></i>重要设施或危险品名称</label></div></li>
					...
					<div class="datagrid-talbe form-datagrid">
						...
						<table class="table table-bordered table-hover" id="hot_list">
							...
							<tr>
								...
								<th class="sort" data-property="content">重要设施或危险品名称<i class="iconfont icon-paixu"></i></th>
...
<script>
	...
	function add() {
		layer.open({
			...
			area: ['100%', '100%'],
			content: cg_vte.path + 'webpage/base_data/dangerousFacilities/dangerousFacilities_add.jsp',
			fix: false,
			scrollbar: false,
		});
	}
	function edit(item) {
		...
		layer.open({
			...
			maxmin: false, //开启最大化最小化按钮
			area: ['100%', '100%'],
			content: cg_vte.path + 'webpage/base_data/dangerousFacilities/dangerousFacilities_edit.jsp',
			btn: ['关闭'],
			fix: false,
			scrollbar: false,
	...
	function deletes() {
		...
		for (var i in ids){
			if (ids.hasOwnProperty(i)){
				params['dangerousFacilities["'+i+'"].id'] = ids[i];
			}
		}

		$.ajax({
			url: cg_vte.path + 'if/dangerousFacilitiesController/dangerousFacilities',
			type: 'post',
			dataType: 'text',
			data: params,
			success: function (data) {
