			...
			<div class="ibox ibox-datagrid">
				...
				<div class="ibox-title">
					...
					<div class="ibox-tools ibox-tools-one">
						<a onclick="add()" ><i class="fa fa-plus"></i> 添加防火单位</a>
						...
						<ul class="dropdown-menu bg-warning" aria-labelledby="dLabel" id="cols">
							...
							<li><div class="checkbox i-checks"><label><input type="checkbox" property="address"><i></i>所在地点</label></div></li>
							<li><div class="checkbox i-checks"><label><input type="checkbox" property="manager"><i></i>管理员</label></div></li>
							<li><div class="checkbox i-checks"><label><input type="checkbox" property="phone"><i></i>联系电话</label></div></li>
							<li><div class="checkbox i-checks"><label><input type="checkbox" property="num_people"><i></i>人数</label></div></li>
							<li><div class="checkbox i-checks"><label><input type="checkbox" property="build_area"><i></i>建筑面积</label></div></li>
						...
						<div class="datagrid-talbe form-datagrid">
							...
							<table class="table table-bordered table-hover" id="hot_list">
								...
								<tr>
									...
									<th class="sort" data-property="phone">联系电话<i class="iconfont icon-paixu"></i></th>
									<th class="sort" data-property="num_people">人数<i class="iconfont icon-paixu"></i></th>
									<th class="sort" data-property="build_area">建筑面积<i class="iconfont icon-paixu"></i></th>
...
<script>
	...
	function add() {
		layer.open({
			...
			area: ['100%', '100%'],
			content: cg_vte.path + 'webpage/base_data/importantunits/importantunits_add.jsp',
			fix: false,
			scrollbar: false,
		});
	}
	function edit(item) {
		...
		layer.open({
			...
			maxmin: false, //开启最大化最小化按钮
			area: ['100%', '100%'],
			content: cg_vte.path + 'webpage/base_data/importantunits/importantunits_edit.jsp',
			btn: ['关闭'],
			fix: false,
			scrollbar: false,
	...
	function deletes() {
		...
		for (var i in ids){
			if (ids.hasOwnProperty(i)){
				params['importantUnits["'+i+'"].id'] = ids[i];
			}
		}

		$.ajax({
			url: cg_vte.path + 'if/importantUnitsController/deleteImportants',
			type: 'post',
			dataType: 'text',
			data: params,
			success: function (data) {
        ...
        for(var i in rows){
            if (rows.hasOwnProperty(i)){
                ...
                AMap.event.addListener(marker, 'click', function(event) {
                    ...
                    info.push("行政区代码 : {pac}");
                    info.push("<br>");
                    info.push("所在地点 : {address}");
                    info.push("<br>");
                    info.push("管理员： {manager}");
                    info.push("<br>");
                    info.push("联系电话： {phone}");
                    info.push("<br>");
                    info.push("人数： {num_people}");
                    info.push("</div>");
