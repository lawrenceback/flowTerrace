		...
		<div class="form-inline">
			<div class=" form-group margin-10 form-group-link fr por"  >
				...
				<ul class="dropdown-menu bg-warning" aria-labelledby="dLabel" id="cols">
					...
					<li><div class="checkbox i-checks"><label><input type="checkbox" checked property="license_status"><i></i>许可状态</label></div></li>
					<li><div class="checkbox i-checks"><label><input type="checkbox" checked property="status"><i></i>许可类型</label></div></li>
					<li><div class="checkbox i-checks"><label><input type="checkbox" property="deviceType"><i></i>设备类型</label></div></li>
					<li><div class="checkbox i-checks"><label><input type="checkbox" property="device"><i></i>设备种类</label></div></li>
		...
		<div class="row">
			<div class="col-md-12">
				<div class="form-inline">
					...
					<div class="form-group margin-10 ">
						<label for="license_status">许可状态：</label>
						<select class="form-control" id="license_status" onchange="this_data.license_status = this.value;initImeiList()">
					...
					<div class="input-group pull-right" >
						<input type="text" class="input form-control text-left" id="key" placeholder="key / imei">
						<span class="input-group-btn">
							<button type="button" class="btn btn btn-primary" onclick="searchLicense()">
		...
		<div class="datagrid-talbe form-datagrid" style="height: calc(100% - 150px);">
			<table class="table table-bordered table-hover" id="imei_list">
				...
				<tr>
					...
					<th data-property="license_status">许可状态</th>
					<th class="sort" data-property="status">许可类型<i class="iconfont icon-paixu"></i></th>
					<th class="sort" data-property="deviceType">设备种类<i class="iconfont icon-paixu"></i></th>
					<th class="sort" data-property="device">设备类型<i class="iconfont icon-paixu"></i></th>
...
<script>
	...
	function initImeiList() {
		...
		$.ajax({
			...
			data: {
                ...
                license_status: this_data.license_status,
			},
			success: function (data) {
				data = JSON.parse(data);
                    ...
                    var sub = temp.temp_imei_list.format({
                        ...
                        license_status: function () {
                            switch (data[i].status){
                                ...
                                case 2:
									//可用
									if (data[i].expTime > new Date().getTime()){
										return '可用';
									}
                                    return '不可用';
                                case 3:return '不可用';
                        ...
                        opt: function () {
                            var btn = '';
                            btn += '<a onclick="settings(this)" title="设置" class="text-primary">[ 设置 ]</a>';
                            btn += ' <a onclick="delImei(this)" title="删除" class="text-primary">[ 删除 ]</a>';
	...
	function settings(item) {
	    ...
	    var tr_data = $(item).parents('tr').data('tr_data');
        window.sessionStorage.setItem('data_tr_license', JSON.stringify(tr_data));
        ...
        layer.open({
            ...
            closeBtn: 0,
            area: ['70%', '70%'],
            content: cg_vte.service_href_no_user + 'base_data/imei/license_setting',
            shadeClose: true,
            fix: false,
            scrollbar: false,
            btn: ['关闭'],
